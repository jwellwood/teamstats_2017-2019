...
export const getFewestGoalsConceded = array => {
  ...
  const fewestConcededNumber = arr => Math.min(...arr.map(a => +a.opponentScore));
  return {
    match: array.filter(a => +a.opponentScore === fewestConcededNumber(removeCleanSheets)),
    value: fewestConcededNumber(removeCleanSheets),
  };
};

export const getCleanSheets = array => array.filter(a => +a.opponentScore === 0);

// Margins

export const getBiggestWinningMargin = array => {
  const getWins = array.filter(a => +a.teamScore > +a.opponentScore);
  const biggestMarginNumber = arr => Math.max(...arr.map(a => +a.teamScore - +a.opponentScore));
  return {
    match: array.filter(a => +a.teamScore - +a.opponentScore === biggestMarginNumber(getWins)),
...
export const getBiggestLosingMargin = array => {
  ...
  const biggestMarginNumber = arr => Math.max(...arr.map(a => +a.opponentScore - +a.teamScore));
  return {
    match: array.filter(a => +a.opponentScore - +a.teamScore === biggestMarginNumber(getLosses)),
